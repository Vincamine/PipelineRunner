...
/**
 * Command for fetching pipeline execution history.
 */
...
public class ReportCommand extends BaseCommand {
    ...
    @CommandLine.Option(names = "--pipeline", description = "Pipeline name to fetch executions for", required = true)
    private String pipeline;

    public ReportCommand(BackendClient backendClient) {
    ...
    public Integer call() {
        try {
            String report = backendClient.getPipelineExecutions(pipeline, outputFormat);

            if (!report.startsWith("Execution History:")) {
            ...
            System.err.println("Error fetching pipeline execution history: " + e.getMessage());
            return 1;
        }
    }
...
// import edu.neu.cs6510.sp25.t1.services.ReportService;
// import edu.neu.cs6510.sp25.t1.models.ReportEntry;
// import edu.neu.cs6510.sp25.t1.utils.ReportErrorHandler;
// import edu.neu.cs6510.sp25.t1.utils.ReportFormatter;
...
// * report --repo https://github.com/user/repo --pipeline main-pipeline --run
// 123
// * --stage build
// *
// */
// @CommandLine.Command(name = "report", description = "Retrieve reports for
// pipeline executions", mixinStandardHelpOptions = true)
// /**
// * ReportCommand class to handle the retrieval of pipeline execution reports.
// */
// public class ReportCommand implements Runnable {

// @CommandLine.Option(names = "--repo", description = "Repository URL to
// retrieve reports from")
// private String repoUrl;

// @CommandLine.Option(names = "--local", description = "Retrieve reports from
// the local repository in the working directory")
// private boolean isLocal;

// @CommandLine.Option(names = "--pipeline", description = "Pipeline Name to
// retrieve reports for")
// private String pipelineId;

// @CommandLine.Option(names = "--run", description = "Unique identifier for a
// specific pipeline execution")
// private Integer runNumber;

// @CommandLine.Option(names = "--stage", description = "Stage name to get
// specific stage report")
// private String stageName;

// @CommandLine.Option(names = "--job", description = "Job name to get specific
// job report")
...
// if ((stageName != null || jobName != null) && (pipelineId == null ||
// runNumber == null)) {
// errors.add("Pipeline name and run number must be specified for stage/job
// reports.");
// }

// // If any validation errors were collected, throw a single exception with all
// // errors listed.
// // This allows the user to see all issues at once instead of fixing them one
// by
// // one.
// if (!errors.isEmpty()) {
...
// List<String> pipelineNames = reportService.getAllPipelineNames(isLocal ? null
// : repoUrl);
...
// String.format("No report found for job '%s' in stage '%s' of run #%d in
// pipeline '%s'",
