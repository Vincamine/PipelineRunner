...
import static edu.neu.cs6510.sp25.t1.model.ReportLevel.SUCCESS;
...
class ReportCommandTest {
    ...
    void testReports_StageReport_Success() {
        ...
        try (MockedStatic<ReportFormatter> reportFormatterMock = mockStatic(ReportFormatter.class)) {
            ...
            assertEquals(0, exitCode, "Stage report should be retrieved successfully.");
            verify(mockReportService).getStageReport(
                    eq("repo-url"), eq("123"), eq(1), eq("stage1")
            );
        }
    }
    ...
    void testReports_MissingRepoUrl() {
        try (MockedStatic<ReportErrorHandler> errorHandlerMock = mockStatic(ReportErrorHandler.class)) {
            ...
            errorHandlerMock.verify(
                    () -> ReportErrorHandler.reportError(eq("Either --repo or --local must be specified.")),
                    times(1)
            );
    ...
    void testReports_JobMissingStage() {
        try (MockedStatic<ReportErrorHandler> errorHandlerMock = mockStatic(ReportErrorHandler.class)) {
            ...
            errorHandlerMock.verify(
                    () -> ReportErrorHandler.reportError(eq("Stage name must be specified when requesting job report.")),
                    times(1)
            );
            assertCommandFailed(exitCode);
        }
    ...
    void testReports_LocalRepository_Success() {
        ...
        when(mockReportService.getLocalPipelineRuns("123"))
                .thenReturn(List.of(mockReport));
    ...
    void testReports_JobReport_Success() {
        ...
        try (MockedStatic<ReportFormatter> reportFormatterMock = mockStatic(ReportFormatter.class)) {
            ...
            verify(mockReportService).getJobReport(
                    eq("repo-url"), eq("123"), eq(1), eq("stage1"), eq("job1")
            );
        }
    }
