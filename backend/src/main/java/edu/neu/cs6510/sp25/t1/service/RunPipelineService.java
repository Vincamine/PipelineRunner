...
public class RunPipelineService {
    ...
    private final Map<String, PipelineStatusResponse> pipelineStatusMap = new HashMap<>();

    public RunPipelineService(WorkerClient workerClient) {
        this.workerClient = workerClient;
    }

    public UUID startPipelineExecution(String repoId, String pipelineId) {
        String pipelineConfigPath = ".pipelines/" + pipelineId + ".yaml";
        File configFile = new File(pipelineConfigPath);

        if (!configFile.exists()) {
            throw new RuntimeException("Pipeline configuration file not found: " + pipelineConfigPath);
        }

        UUID pipelineRunId = UUID.randomUUID();
        pipelineStatusMap.put(pipelineRunId.toString(), new PipelineStatusResponse(pipelineRunId.toString(), "Running"));

        workerClient.sendJobToWorker(repoId, pipelineId, pipelineConfigPath);

        return pipelineRunId;
    }

    public PipelineStatusResponse getPipelineStatus(String repoId, String pipelineId) {
        return pipelineStatusMap.getOrDefault(pipelineId, new PipelineStatusResponse(pipelineId, "Not Found"));
    }
}
...
//      */
//     public PipelineExecution executePipeline(Map<String, Object> pipelineConfig) {
//         try {
//             // Validate pipeline structure
//             if (!validator.validatePipeline(pipelineConfig)) {
//                 System.err.println("Pipeline validation failed.");
//                 return new PipelineExecution("unknown", PipelineState.FAILED, 0, "Validation error.");
//             }
...
//             final PipelineExecution status = new PipelineExecution("pipeline1", PipelineState.RUNNING, 0,
//                     "Executing pipeline.");
...
//                     return new PipelineExecution("pipeline1", PipelineState.FAILED, 1, "Pipeline execution failed.");
//                 }
//             }

//             System.out.println("Pipeline execution complete.");
//             return new PipelineExecution("pipeline1", PipelineState.SUCCESS, 0, "Pipeline executed successfully.");
//         } catch (Exception e) {
//             ErrorHandler.reportError("Pipeline execution error: " + e.getMessage());
//             return new PipelineExecution("pipeline1", PipelineState.FAILED, 1, "Execution error.");
//         }
//     }
