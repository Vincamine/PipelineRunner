...
public class JobStatusCacheService {
  ...
  @Autowired
  public JobStatusCacheService(StringRedisTemplate redisTemplate) {
   ...
   */
  public Optional<ExecutionStatus> getCachedJobStatus(String jobId) {
    try {
      String status = redisTemplate.opsForValue().get(JOB_STATUS_PREFIX + jobId);
      return Optional.ofNullable(status).map(ExecutionStatus::valueOf);
    } catch (Exception e) {
   ...
   */
  public void cacheJobStatus(String jobId, ExecutionStatus status) {
    try {
